# need season id
# probably a function to get the upcoming season id and add it to an existing file
...
def get_new_seasons():
    # no input
    old_seasons = pd.read_csv("/users/jordanwegner/Desktop/nba2/03_data/seasons.csv") # update this path
    ...
    if any(old_seasons['id'].str.contains(current_seasons)):
        ...
        if (int(y1)-int(updated_seasons['y1'][len(updated_seasons)-2]))!=1:
            ...
            old_file_id = "/users/jordanwegner/Desktop/nba2/03_data/00_archive/seasons_"+ (datetime.datetime.today() - timedelta(1)).strftime('%Y%m%d')+".csv" # updated this path
            old_seasons.to_csv(old_file_id, index=False)
            updated_seasons.to_csv('/users/jordanwegner/Desktop/nba2/03_data/seasons.csv',index=False) # update this path